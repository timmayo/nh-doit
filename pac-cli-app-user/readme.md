...
| [`setup.md`](./setup.md) | Full setup sequence — running the two PowerShell scripts, Azure Automation Account configuration, IAM role assignments, and Power Automate flow setup, with architecture diagrams. |
| `solution/src/` | Unpacked solution source (via `pac solution unpack`). Version controlled. |
| `solution/releases/NHDoIT_1_0_0_x_managed.zip` | Managed solution package for import into the customer's environment. |
| `solution/releases/NHDoIT_1_0_0_x_unmanaged.zip` | Unmanaged solution package, for development/customization. |

---

## Run Order

1. Review [`setup.md`](./setup.md) and ensure all Azure assets are in place
...
- This customer's tenant is GCC (moderate) — Entra ID/ARM calls use commercial endpoints, while Power Platform/Dataverse calls use GCC-specific endpoints. See [`setup.md`](./setup.md) for the full explanation.